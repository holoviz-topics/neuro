...
def generate_spike_timeseries(duration: float, sampling_rate: float = 30000, 
                              spike_rate_range: Tuple[float, float] = (.1, 20), 
                              gaussian_std_dev: float = 0.1, 
                              decay_rate: float = 5.0, 
                              spike_dur: float = 0.001, 
                              amplitude: float = 100.0) -> np.ndarray:
    """
    ...
    amplitude (float, optional): Amplitude scaling factor for the generated spike data
        (default is 100.0 microvolts).
    ...
    spike_rate = np.random.uniform(*spike_rate_range)  # Average spike rate (spikes per second)
    ...
    spike_waveform = generate_action_potential(np.linspace(-1, 2, int(sampling_rate*spike_dur)), gaussian_std_dev, decay_rate) 
    ...
    valid_spike_times = [spike_times[i] for i in range(len(interspike_interval)) if interspike_interval[i] >= len(spike_waveform)]
    valid_spike_times.append(spike_times[-1]) # include the last spike

    spikes = np.zeros(len(time))
    for spike_time in valid_spike_times:
        if spike_time + len(spike_waveform) < len(spikes):  # make sure not to go past the end of the array
            spikes[spike_time:spike_time+len(spike_waveform)] += spike_waveform * amplitude  # Adjust the amplitude to a reasonable range (in microvolts)
    
    return spikes

def generate_ephys_powerlaw(n_channels: int, n_seconds: float, fs: int = 30000,
                            highpass: float = 2.0, exponent: float = -1,
                            amplitude: float = 20.0) -> tuple[np.ndarray, np.ndarray, list]:
    """
    Generate synthetic ephys data as power law time series at a specified exponent and poisson spikes.

    Args:
        n_channels (int): Number of ephys channels.
        n_seconds (float): Duration of the ephys data in seconds.
        fs (int): Sampling rate of the ephys data in Hz (default is 30000).
        ...
        amplitude (float, optional): Amplitude scaling factor for the generated ephys data.
            Defaults to 20.0 microvolts.

    Returns:
        data (np.ndarray): Synthetic ephys data as a NumPy array of shape (n_channels, total_samples).
    ...
    for ch in range(n_channels):
        pl_noise = sim_powerlaw(n_seconds, fs, exponent=exponent, f_range=(highpass, None)) * amplitude
    ...
    assert ephys.shape == (n_channels, total_samples), f"Incorrect dimensions {ephys.shape} for data array {(n_channels, total_samples)}"
...
def create_ephys_channel_names(n_channels: (int) = None) -> list[str,]:
    """ Given number of channels, return list of str like ['1', '2', ...]"""
